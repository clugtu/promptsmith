...
class TestReferenceSheetIntegration:
    ...
    def test_garou_reference_sheet_page_1(self, custom_path):
        ...
        # Generate all prompts for page 1 (first 4 characters)
        blocks = []
        characters = json_data.get("characters", [])[:4]  # First page = first 4 characters
        
        for char_data in characters:
            character_id = char_data.get("id") or char_data.get("name")
            character_name = char_data.get("name")  # Use name for block labels
            character_archetype = char_data.get("archetype", "")
            char_gender = char_data.get("gender", None)
            char_proportions = char_data.get("proportions", "").strip()
            ...
            for pose in poses:
                ...
                # Resolve prompt
                p0, thematic_snip, gender_for_prompt, ref_proportions, age, equipment, pose_prompt, camera_rotation, visual_notes = \
                    create_image.resolve_prompt_from_json(json_data, character=character_id, form=pose_name)
                ...
                blocks.append(f"[{character_name}:{pose_name}]\n{create_image.sanitize_for_ascii(create_image.format_for_chat(p))}")
